...
path = 'C:\\Users\\Rabbit\\Documents\\GitHub\\insta-schedule\\posts'
                ...
                if todays_date.hour == POST_HOUR:
                    ...
                    if not post.has_been_posted: 
                        with client(USERNAME, PASSWORD) as cli:
                            ...
                            nt("Posted {0} {1} {2}...".format(todays_date, post.image, post.description))
                            post.has_been_posted = True
                    with open('posts.json', 'w') as json_file:  
                        json.dump(POST_LIST.serialize(), json_file)
                    
        print("Listening {0} ...".format(todays_date))
        time.sleep(60)
